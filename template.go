// code from https://github.com/cloudfoundry/bosh-cli without bosh-cli dependencies
package yint
...
func (i interpolator) Interpolate(node interface{}, varsLookup varsLookup) (interface{}, error) {
	...
	case string:
		for _, name := range i.extractVarNames(typedNode) {
			...
			if err != nil {
				return nil, fmt.Errorf("Finding variable '%s': %s", name, err.Error())
			}
...
func (t varsTracker) Get(name string) (interface{}, bool, error) {
	...
	if err != nil {
		return nil, false, fmt.Errorf("Interpolating variable '%s' definition options: %s", name, err.Error())
	}
...
func (t varsTracker) scopedVarsTracker(name string) (varsTracker, error) {
	if _, found := t.visited[name]; found {
		return varsTracker{}, errors.New("Detected recursion")
	}
...
func (t *varsTracker) ExtractDefinitions(obj interface{}) error {
	...
	for _, def := range t.defs.Definitions {
		if len(def.Type) > 0 {
			...
			if err != nil {
				return fmt.Errorf("Getting all variables from variable definitions sections: %s", err.Error())
			}
		}
	}
...
func (t varsTracker) FoundVars(obj interface{}) (interface{}, error) {
	...
	for _, def := range t.defs.Definitions {
		...
		if err != nil {
			return nil, fmt.Errorf("Getting all variables from variable definitions sections: %s", err.Error())
		}
		if found {
...
func (t varsTracker) MissingError() error {
	...
	return fmt.Errorf("Expected to find variables: %s", t.multiErr(t.missing))
}
...
func (t varsTracker) ExtraError() error {
	...
	return fmt.Errorf("Expected to use variables: %s", t.multiErr(unusedNames))
}
...
func (t varsTracker) multiErr(mapWithNames map[string]struct{}) error {
	...
	for _, name := range names {
		err = multierror.Append(err, errors.New(name))
	}
	err.ErrorFormat = errFormatFunc
	return err
}
...
func errFormatFunc(es []error) string {
	if len(es) == 1 {
